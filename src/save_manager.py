...
class SaveManager:
    ...
    def init_repository(self) -> None:
        path = f'{SAVES_PATH}/{self.save_name}/chunks'
        os.makedirs(path, exist_ok=True)

    def load_chunk(self, id: int) -> Chunk|None:
        try:
            with open(f'{SAVES_PATH}/{self.save_name}/chunks/{id}.json') as f:
    ...
    def save_chunk(self, chunk: Chunk|None) -> None:
        ...
        with open(f'{SAVES_PATH}/{self.save_name}/chunks/{chunk.id}.json', 'w') as f:
